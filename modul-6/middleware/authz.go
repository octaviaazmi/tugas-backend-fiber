...
// RequirePermission dipasang pada endpoint yang haknya bisa diputuskan
// TANPA menyentuh data (lihat A.6 di modul).
func RequirePermission(perms *helper.PermissionSet, permission string) fiber.Handler {
	return func(c *fiber.Ctx) error {
		user, ok := helper.CurrentUser(c)
		if !ok {
			// Sampai sini tanpa identitas berarti RequireAuth belum dipasang.
